...
- **`tasks/skills.sh`** — Repo-level agent-skill linking. Scans `~/project` for gitignored `.local-skills.{json,yml,yaml}` manifests and, per repo, symlinks the named skills (resolved from `~/.config/agent-skills/`) into `<repo>/.claude/skills/` and renders a gitignored `mise.local.toml` from the manifest's `auth.config` — **non-secret env only** (e.g. `JIRA_CONFIG_FILE`); secrets are never put in mise (it evaluates `[env]` on every `cd`, so a blocking `op read` would hang the shell). For a `jira` block it prints one-time setup commands: load the API token into the macOS Keychain from 1Password (`security add-generic-password … "$(op read …)"`, where jira-cli reads it at runtime) and `jira init`. Used to scope skills that should not be globally active (`jira`, Google Cloud skills). Idempotent; never auto-run.
