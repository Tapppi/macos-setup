...
# tasks/macos.sh — https://mths.be/macos

# Close any open System Settings panes, to prevent them from overriding
# settings we're about to change
osascript -e 'tell application "System Settings" to quit'

# Start sudo keep-alive only when running standalone (not sourced from setup.sh)
...
defaults write com.apple.sound.beep.sound -string /System/Library/Sounds/Sosumi.aiff

# Disable UI and feedback audio
...
# Mouse: pointer tracking speed (0–3)
...
# Mouse: disable acceleration (linear tracking)
defaults write NSGlobalDomain com.apple.mouse.linear -int 1

# Magic Mouse: button mode and gestures
# MouseButtonMode "OneButton" = whole surface acts as one button (no left/right split)
# MouseTwoFingerDoubleTapGesture 3 = Smart Zoom
# MouseTwoFingerHorizSwipeGesture 2 = swipe between pages
...
# Disable Sonoma keyboard layout popup in input fields
defaults write kCFPreferencesAnyApplication TSMLanguageIndicatorEnabled 0

# Use F1-F12 as standard function keys; hold fn for media and brightness
...
# Enable keyboard input sources: Finnish, Finnish Programmer (custom), and U.S.
# Finnish-prog bundle is installed by bootstrap.sh → ~/Library/Keyboard Layouts/
# Uses python3+plistlib (same approach as notification prefs below) because
# defaults(1) cannot write arrays of dicts.
python3 - <<'PYEOF'
import plistlib, os
...
keyboard_layouts = [
    ...
    {
        "InputSourceKind": "Keyboard Layout",
        "KeyboardLayout ID": -14667,
        "KeyboardLayout Name": "Finnish-prog",
    },
    {
        "InputSourceKind": "Keyboard Layout",
        "KeyboardLayout ID": 0,
...
non_keyboard = [
    s
    for s in prefs.get("AppleEnabledInputSources", [])
    if s.get("InputSourceKind") != "Keyboard Layout"
]

prefs["AppleEnabledInputSources"] = keyboard_layouts + non_keyboard
...
# NOTE: systemsetup logs a cosmetic Error:-99 from InternetServices.m on
# Sequoia/Tahoe but still sets the timezone. No non-deprecated replacement
# exists — Apple still documents this command for Tahoe 26.
sudo systemsetup -settimezone "Europe/Helsinki" > /dev/null 2>&1
# Verify timezone was applied
if ! sudo systemsetup -gettimezone 2>/dev/null | grep -q "Europe/Helsinki"; then
	echo "Warning: timezone may not have been set correctly" >&2
fi
...
# NOTE: no pmset equivalent exists; pmset autorestart covers power loss only
sudo systemsetup -setrestartfreeze on > /dev/null 2>&1

# Sleep the display after 10 minutes on battery and 20 while charging
...
# Suppress notifications when mirroring or sharing the display
# Replaces Muzzle app — native macOS feature since Ventura/Sonoma
# System Settings → Notifications → "Allow notifications when mirroring or sharing the display" = OFF
# dndMirrored=true means DND activates automatically when display is being shared/mirrored
python3 - <<'PYEOF'
import plistlib, os
plist_path = os.path.expanduser('~/Library/Preferences/com.apple.ncprefs.plist')
with open(plist_path, 'rb') as f:
    outer = plistlib.load(f)
dnd_blob = outer.get('dnd_prefs')
inner = plistlib.loads(dnd_blob) if dnd_blob else {}
inner['dndMirrored'] = True
outer['dnd_prefs'] = plistlib.dumps(inner, fmt=plistlib.FMT_BINARY)
with open(plist_path, 'wb') as f:
...
# This is under SIP control since 10.13
# Disable SIP by running `. csrutil disable` in recovery mode
...
# Force US keyboard layout when iTerm2 is focused
# (useful when using a non-US layout like Finnish as the system default)
defaults write com.googlecode.iterm2 ForceKeyboard -bool true
defaults write com.googlecode.iterm2 KeyboardLocale -string "com.apple.keylayout.US"
...
# Modified to a more modern alternative
...
echo "Done configuring macOS. Note that some of these changes require a logout/restart to take effect."
