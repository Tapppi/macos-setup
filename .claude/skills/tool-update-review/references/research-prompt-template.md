...
prompt (SKILL.md step 3). This exists because the prompt used to get
...
quality, watch items, ...) live in SKILL.md step 3 and are deliberately
...
in. Read SKILL.md step 3 in full before filling this in; this is not a
substitute for it.

One subagent gets one filled-in copy of this template — whether it's
covering a single tool (individual-focus tier) or a batch (SKILL.md step
3's tiering).
...
references/design.md A.1, and write the complete array (one element per
...
general awareness of what's already changed recently (design.md C.5):
{{REPO_CONTEXT_JSON}}

Paths you may scan for relevancy (design.md B.2's "Relevancy is the point
of this skill"):
...
Audit trail to check for config_status and watch items (design.md C.5, C.7):
    ${XDG_STATE_HOME:-~/.local/state}/tool-update-review/changelog.md
    ${XDG_STATE_HOME:-~/.local/state}/tool-update-review/watch-items.json

Follow SKILL.md step 3's full research quality bar — headliner atomicity
...
cross-referencing, bespoke tasks/*.sh setup handling (C.6) — read it before
you start, not after. Hold yourself to the exact schema shapes in
design.md A.1 (headliners as {text,category,severity} objects, relevancy
...
See SKILL.md step 3 for the individual-vs-batch heuristic (word-boundary
